...
ah_list = ['FC(F)(F)c1ccc(Cl)cc1', 'FC(F)(F)c1ccc(Br)cc1', 'FC(F)(F)c1ccc(I)cc1',
           'COc1ccc(Cl)cc1', 'COc1ccc(Br)cc1', 'COc1ccc(I)cc1',
           'CCc1ccc(Cl)cc1', 'CCc1ccc(Br)cc1', 'CCc1ccc(I)cc1',
           'Clc1ccccn1', 'Brc1ccccn1', 'Ic1ccccn1',
           'Clc1cccnc1', 'Brc1cccnc1', 'Ic1cccnc1']


def get_train_test_index(split_mode):
    if split_mode == 1:
        ah_test = ['FC(F)(F)c1ccc(Cl)cc1', 'FC(F)(F)c1ccc(Br)cc1', 'FC(F)(F)c1ccc(I)cc1']
    elif split_mode == 2:
        ah_test = ['COc1ccc(Cl)cc1', 'COc1ccc(Br)cc1', 'COc1ccc(I)cc1']
    elif split_mode == 3:
        ah_test = ['CCc1ccc(Cl)cc1', 'CCc1ccc(Br)cc1', 'CCc1ccc(I)cc1']
    elif split_mode == 4:
        ah_test = ['Clc1ccccn1', 'Brc1ccccn1', 'Ic1ccccn1']
    elif split_mode == 5:
        ah_test = ['Clc1cccnc1', 'Brc1cccnc1', 'Ic1cccnc1']
    else:
        raise ValueError("Unknown split_mode: {}".format(split_mode))
    ah_train = list()
    for ah in ah_list:
        if ah not in ah_test:
            ah_train.append(ah)

    index = pandas.read_csv("../datasets/real/real_4/real_4.csv")

    def get_subset(ar):
        a = index[index['Aryl_halide_SMILES'] == ar[0]]
        b = index[index['Aryl_halide_SMILES'] == ar[1]]
        to_return = pd.concat([a, b])

        for i in range(2, len(ar)):
            c = index[index['Aryl_halide_SMILES'] == ar[i]]
            to_return = pd.concat([to_return, c])
        return to_return

    index_train = get_subset(ah_train)
    index_test = get_subset(ah_test)

    set_train = set(index_train.index.tolist())
    set_test = set(index_test.index.tolist())
    assert (len(set_train & set_test) == 0)
    assert (len(set_train | set_test) == 3955)

    return np.array(index_train.index.tolist()), np.array(index_test.index.tolist())
...
def main():
    sms = [0]
    reprs = ["pka_bde01", "morgan_fp", "rxnfp"]

    for split_mode in sms:
        if split_mode != 0:
            train_idx, test_idx = get_train_test_index(split_mode)
        else:
            train_idx, test_idx = random_dataset(3955, 0.0003)
        assert (train_idx.shape[0] + test_idx.shape[0] == 3955)
        for representation in reprs:
            ...
            if args.representation == "one_hot":
                ...
                flattened_encoding_of_dataset, yields = pka_bde.encode_dataset(sheet, num_columns_of_sheet, args, False, 4)
                ...
                flattened_encoding_of_dataset, yields = pka_bde.encode_dataset(sheet, num_columns_of_sheet, args, True, 4)
