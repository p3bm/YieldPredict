...
class Arguments:
    def __init__(self):
        ...
        self.representations = ['morgan_fp', 'Mordred']
...
def main():
    ...
    for i in range(len(args.representations)):
        ...
        if reduce_method is not None:
            ...
            orig_x_train = xs_reduced
        print(orig_x_train.shape)
        origx_trains.append(orig_x_train)

    print(orig_y_train_unnormalized.shape)
    full_y = np.concatenate((orig_y_train_unnormalized, orig_y_test_unnormalized), axis=0)
    ...
    print(all_low0.shape)
    print(full_y.shape)
    
    def double_cluster(dec0, dec1, data0, data1, start_id, step_size, n_clusters = 30, epoch_size = 20, dir_name = 'figs', const_epoch = False, al_method = 'prob', final =False):
        ...
        train_idx = start_id
        test_idx = [i for i in range(len(full_y)) if i not in start_id]
        ...
        def save_test():
            ...
            def saveimg():
                ys = [orig_0_f1, orig_1_f1, cluster_0_f1, cluster_1_f1, encode_0_f1, encode_1_f1]
                xs = [orig_0_maes, orig_1_maes, cluster_0_maes, cluster_1_maes, encode_0_maes, encode_1_maes]
                labels = ['orig_0_f1', 'orig_1_f1', 'cluster_0_f1', 'cluster_1_f1', 'encode_0_f1', 'encode_1_f1']
                fig, ax = plt.subplots()
                for i in range(6):
                    y = [1/yyy for yyy in ys[i]]
                    x = xs[i]
                    labelll = labels[i]
                    ax.plot(x, y, label = labelll)
                ax.legend()
                plt.savefig('./figs/' + dir_name + '/' 
                            + str(n_clusters) + '_' 
                            + str(len(start_id)) + '_' 
                            + str(epoch_size) + al_method + '.png')
            saveimg()

            with open("./results/r2.csv","a+") as f:
                f.write(str(n_clusters) + ',')
                f.write(str(len(start_id)) + ',')

                f.write(str(orig_0_r2) + ',')
                f.write(str(orig_0_mae) + ',')
                f.write(str(orig_0_f1) + ',')
                f.write(str(orig_0_rmse) + ',')
                f.write(str(orig_0_maes) + ',')

                f.write(str(orig_1_r2) + ',')
                f.write(str(orig_1_mae) + ',')
                f.write(str(orig_1_f1) + ',')
                f.write(str(orig_1_rmse) + ',')
                f.write(str(orig_1_maes) + ',')

                f.write(str(cluster_0_r2) + ',')
                f.write(str(cluster_0_mae) + ',')
                f.write(str(cluster_0_f1) + ',')
                f.write(str(cluster_0_rmse) + ',')
                f.write(str(cluster_0_maes) + ',')

                f.write(str(cluster_1_r2) + ',')
                f.write(str(cluster_1_mae) + ',')
                f.write(str(cluster_1_f1) + ',')
                f.write(str(cluster_1_rmse) + ',')
                f.write(str(cluster_1_maes) + ',')

                f.write(str(encode_0_r2) + ',')
                f.write(str(encode_0_mae) + ',')
                f.write(str(encode_0_f1) + ',')
                f.write(str(encode_0_rmse) + ',')
                f.write(str(encode_0_maes) + ',')

                f.write(str(encode_1_r2) + ',')
                f.write(str(encode_1_mae) + ',')
                f.write(str(encode_1_f1) + ',')
                f.write(str(encode_1_rmse) + ',')
                f.write(str(encode_1_maes) + ',')

                f.write(str(epoch_size) + ',')
                f.write(al_method + '\n')
            if final:
                with open("./results/r2_real7.csv","a+") as f:
                    f.write(str(n_clusters) + ',')
                    f.write(str(len(start_id)) + ',')
            ...
            def save_all_y():
                pred_array = dict()
                pred_array[f'orig_0_{round}'] = y_orig_0
                pred_array[f'orig_1_{round}'] = y_orig_1
                pred_array[f'cluster_0_{round}'] = y_cluster_0
                pred_array[f'cluster_1_{round}'] = y_cluster_1
                pred_array[f'encode_0_{round}'] = y_encode_0
                pred_array[f'encode_1_{round}'] = y_encode_1
                file_path = './figs/' + dir_name + '/' + str(n_clusters) + '_' + str(len(start_id)) + '_' + str(epoch_size) + al_method + '.npz'
                np.savez(file=file_path, **pred_array)
            save_all_y()
    ...
    def activate_learning():
        ...
        train_id = [i for i in range(num_orig_train)]
        print(train_id)
        step_size = 21
        total_size = 168
        ...
        #    add_id = double_cluster(dec0, dec1, data0, data1, train_id, step_size, 20, 60, dir_name)
        #    train_id = train_id + add_id
        double_cluster(dec0, dec1,data0, data1, train_id, step_size, 120, 400, dir_name,final=True)
